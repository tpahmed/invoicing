    ...
    useEffect(()=>{
        if(Facture.invoiceTo){
            ...
            axios.get(`//localhost:4444/client/${Facture.invoiceTo}`).then((e)=>{
                SetClient(e.data.data)
            });
            axios.get(`//localhost:4444/societe/${Facture.payTo}`).then((e)=>{
            ...
            <div className="AfficherFacture-Facture-Donne" ref={FactureRef}>
                ...
                <div className='AfficherFacture-Facture-Table'>
                    <table>
                        ...
                        <tbody>
                            {CommandFacture.map(e=>(
                                <tr>
                                    <td>{e.categorie_title}</td>
                                    <td>{e.product_description}</td>
                                    <td>{e.Qty}</td>
                                    <td>${e.product_price}</td>
                                    <td>${e.product_price*e.Qty}</td>
                                </tr>
                            ))}
